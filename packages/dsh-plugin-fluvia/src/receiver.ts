...
import { renderStatusJson, renderStatusPage, type StatusView } from './status-page.js'
 ...
 */
export function startReceiver(options: ReceiverOptions): Promise<Receiver> {
  ...
  const server = createServer((request, response) => {
    ...
    handle(request, response, options, startedAt).catch((error: unknown) => {
  ...
  return new Promise<Receiver>((resolve, reject) => {
    ...
    const onBindError = (error: Error): void => reject(new Error(`cannot bind ${options.host}:${options.port} — ${error.message}`))
    ...
    server.listen(options.port, options.host, () => {
      ...
      server.on('error', (error: Error) => options.log.warn(`server error: ${error.message}`))
 ...
 */
async function handlePost(request: IncomingMessage, response: ServerResponse, options: ReceiverOptions): Promise<void> {
  let body: string
  try {
...
function view(options: ReceiverOptions, startedAt: number): StatusView {
  return {
    host: options.host,
    port: options.port,
    path: options.path,
...
function trySend(response: ServerResponse, status: number, type: string, body: string): void {
  try {
    if (!response.headersSent) send(response, status, type, body)
    else response.end()
  } catch {
