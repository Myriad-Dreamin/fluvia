 ...
 */
export function buildFluviaTool(cli: FluviaCli): ToolDefinition {
  return defineTool({
    name: 'fluvia',
    ...
    async execute(args, exec) {
      ...
      if (!raw) {
        return { text: `Empty line. ${SYNTAX_REMINDER}`, kind: 'error' as const }
      }
 ...
 */
export function renderAnswer(answer: CliAnswer): FluviaToolValue {
  if (answer.type === 'ack') {
