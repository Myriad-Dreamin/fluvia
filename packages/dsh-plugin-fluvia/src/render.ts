...
export class EnvelopeCoalescer {
  private buffer: WireNotification[] = []
  private window: NodeJS.Timeout | undefined
  ...
  constructor(
    ...
    private readonly emit: (envelope: NotifyEnvelope) => void,
  ) {}
  ...
  add(notification: WireNotification): void {
    ...
    if (this.closed || this.buffer.length >= MAX_BATCH) this.flush()
    else this.arm()
  }
 ...
 */
export function buildEnvelope(batch: WireNotification[], agent: string, session: string): NotifyEnvelope {
 ...
 */
export function renderBlock(sorted: WireNotification[], agent: string, session: string): string {
...
function headline(sorted: WireNotification[], exits: WireNotification[] = []): string {
  ...
  const order: [WireNotification['outcome'], string][] = [
    ['done', 'ready'],
    ['failed', 'failed'],
    ['skipped', 'skipped'],
    ['cancelled', 'cancelled'],
...
function renderDone(n: WireNotification): string {
...
function renderFailed(n: WireNotification): string[] {
...
function renderSkipped(n: WireNotification): string {
...
function renderCancelled(n: WireNotification): string {
 ...
 */
function renderRunnable(sorted: WireNotification[]): string[] {
 ...
 */
function renderExited(n: WireNotification): string[] {
  const p = n.process
  const name = n.ready?.name ?? n.bind.value
...
function timing(n: WireNotification): string {
