 ...
 */
export function parseAddress(spec) {
    if (spec.startsWith('unix:'))
        return { kind: 'unix', path: spec.slice('unix:'.length) };
    if (spec.startsWith('tcp:')) {
        const rest = spec.slice('tcp:'.length);
        const index = rest.lastIndexOf(':');
        if (index <= 0)
            throw new Error(`${spec}: expected tcp:<host>:<port>`);
        const port = Number(rest.slice(index + 1));
        if (!Number.isInteger(port) || port <= 0 || port > 65535)
            throw new Error(`${spec}: bad port`);
...
export class WireConnection {
    ...
    async submit(line, signal) {
        ...
        if (!socket || socket.destroyed)
            throw new Error('the fluvia runtime connection is not writable');
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                cleanup();
                reject(new Error(`could not write to the fluvia runtime: ${error.message}`));
            }
        });
    }
    ...
    async open() {
        ...
        const welcome = await new Promise((resolve, reject) => {
            let greeting = '';
            const onError = (error) => reject(new Error(`handshake failed: ${error.message}`));
            const timer = setTimeout(() => {
                socket.destroy();
        ...
        log.info(`connected to ${formatAddress(address)} as "${welcome.agent}" (runtime session ${welcome.session}, ${welcome.isa.length} instructions, concurrency ${welcome.concurrency})`);
    ...
    attach(socket) {
        ...
        socket.on('error', (error) => this.onDisconnect(error.message));
    }
...
export class WireHub {
     ...
     */
    constructor(address, tokenFile, labelFor, onNotification, log) {
    ...
    async readToken() {
        ...
        try {
            ...
            throw new Error(`cannot read tokenFile "${this.tokenFile}": ${error.message}`);
        }
    }
}
