 ...
 */
export function renderDescription(isa) {
    ...
    if (async_.length > 0) {
        ...
        sections.push('', `Instructions loaded by the host (${async_.length}); each binds <out>N and errN:`, ...listed.map((entry) => `  ${entry.name}${entry.params} → ${entry.out}N, errN — ${entry.summary}`));
        if (async_.length > listed.length) {
            sections.push(`  …and ${async_.length - listed.length} more; run \`defs\` for the full list.`);
        }
    }
    if (control.length > 0) {
        sections.push('', `Control instructions (answer with text, bind no handles): ${control.map((entry) => entry.name).join(', ')}.`);
    }
    sections.push('', 'The instruction set is fixed by the host that runs the fluvia runtime. You cannot add to it, change it, or reach the implementations behind it — submit lines against what is listed above.', 'The long form is skills/fluvia/SKILL.md in the fluvia repository.');
 ...
 */
export function renderLineDescription(isa) {
 ...
 */
export function buildFluviaTool(hub, isa) {
    return defineTool({
        name: 'fluvia',
        ...
        async execute(args, exec) {
            ...
            if (/[\r\n]/.test(raw))
                return { text: `Submit one line per call. ${SYNTAX_REMINDER}`, kind: 'error' };
