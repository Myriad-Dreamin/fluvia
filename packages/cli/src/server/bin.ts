 ...
 *
 * Usage:
 *   fluvia serve --listen unix:/run/fluvia.sock --preload <module>
 *                [--concurrency 4] [--trace out/serve.jsonl.gz]
 *                [--token-file <path>] [--scope agent|runtime]
 *
 * @module @fluvia/cli/server/bin
 */
...
import type { CallOutcome, SessionStats } from '@fluvia/core/types'
import { FunctionRegistry } from '@fluvia/core/plugins/registry'
...
import { formatAddress, parseAddress } from '@fluvia/core/protocol'
import { DEFAULT_TOOLBOX, preload } from '../preload.ts'
import { VERSION } from '../version.ts'

/** Run the out-of-sandbox runtime against `argv` (everything after `fluvia serve`). */
export async function main(argv: string[]): Promise<void> {
  ...
  for (const specifier of values.preload?.length ? values.preload : [DEFAULT_TOOLBOX]) {
    await preload(ctx.functions, specifier)
  }
  ...
  tracer.emit('session.start', {
    meta: {
      ...
      runtime: { node: process.version, platform: process.platform, fluvia: VERSION },
      concurrency,
  ...
  // The listener owns the process from here: resolve only when it is torn down.
  ...
  function announce(): void {
    ...
    process.stdout.write(
      [
        ...
        '',
        `  connect:    fluvia connect --connect ${formatAddress(address)}`,
        '',
      ].join('\n'),
    )
  }
