 ...
 *
 * Usage: fluvia dsh-inbox [--port 7788] [--path /inbox] [--log <file>]
 *
 * @module @fluvia/cli/dsh-inbox/bin
 */
...
/** Run the inbox against `argv` (everything after `fluvia dsh-inbox`). */
export async function main(argv: string[]): Promise<void> {
  ...
  if (values.help) {
    process.stdout.write('usage: fluvia dsh-inbox [--port 7788] [--host 127.0.0.1] [--path /inbox] [--log <file>]\n')
    return
  }
  ...
  // The listener owns the process from here.
