/**
 * `fluvia demo` — drives the fluvia CLI the way an agent would.
 *
 * This is not an in-process simulation: it spawns `fluvia cli` as a real
 ...
 *
 * - `FLUVIA_CLI` — path to the `fluvia` executable, default this package's bin;
 * - `FLUVIA_DEMO_TIMEOUT` — global deadline in ms, default 60000.
 *
 * @module @fluvia/cli/demo/run
 */
...
/**
 * Where `out/` lives. The demo is a workspace command, so the directory it was
 * invoked from is the right place for what it records.
 */
const ROOT = process.cwd()

/** This package's `fluvia` executable, so the demo needs nothing on PATH. */
const FLUVIA_BIN = fileURLToPath(new URL('../../bin/fluvia.js', import.meta.url))
...
class Demo {
  ...
  async start(): Promise<void> {
    ...
    // process.execPath keeps the child on the same Node this driver runs on.
...
export async function main(argv: string[] = []): Promise<void> {
  ...
  const cliPath = process.env.FLUVIA_CLI ?? FLUVIA_BIN
