 ...
 *
 * Usage: tsx src/client/bin.ts [--connect unix:/tmp/fluvia.sock] [--label me]
 *                              [--token-file <path>] [--script <file>]
 *
 * @module fluvia/client/bin
 */
...
import { parseAddress } from '../server/protocol.ts'
import { ms, renderNotificationLine } from '../cli/format.ts'
...
if (values.help) {
  process.stdout.write('usage: tsx src/client/bin.ts [--connect <addr>] [--label <name>] [--token-file <p>] [--script <f>]\n')
  process.exit(0)
}
...
/** Exported so the module reads as a program, not a library. */
export const started = ms(0)
